...
namespace PRoConEvents
{
    ...
    class TeamspeakSync : PRoConPluginAPI, IPRoConPluginInterface
    {
        ...
        Boolean dbgTSSyncPromote = false;
        ...
        List<String> mTSSyncedUsers = new List<String>(); //PURE
        // -- Channel Information ---------------------------------------------
        ...
        // -- TSSync Promotion Variables (PURE) -------------------------------
        Boolean mEnableTSPromotion = true;
        int mMaxSeedingRecognitionVal = 3;
        String mSeedingRecognitionMessage = "Thanks [USER] for helping seed the [TEAM] team's squad-synced TeamSpeak Channels![NEWLINE]Type !teamspeak to join forces";
        int mMinSyncedPlayersForInGamePromotion = 5;
                    ...
                    "<h2>Setup Instructions</h2>" +
                     ...
                     "<h2>Settings</h2>" +
                       ...
                       "<h3>Section 7 - Debug Information</h3>" +
                        ...
                        "<h3> Section 9 - TeamSpeak Sync Promotion</h3>" +
                        "<p>This section contains controls relevant to the custom TeamSpeak Sync Promotion functionality</p>" +
                        "<h5>In-game seeding recognition</h5>" +

                        "<p>When a player joins a squad sync channel AND there are <= 3 squad-sync players (including them) on their team:" +
                        "<br/>Thanks to <name> for helping seed the <team> team's squad-synced TeamSpeak channels! Type !teamspeak to join forces and kick some ass.</p>" +

                        "<h5>In-game promotion of squad-sync usage</h5>" +

                        "<p>Every 20 minutes:" +
                        "<br/>If >= 5 squad sync users online for EACH team:" +
                        "<br/><#> players are having fun and kicking ass with their squadmates right now using our squad-synced TeamSpeak channels!  Type !teamspeak to join.</p>" +

                        "<h4>Enable TSSync Promotion</h4>" +
                           "<blockquote style=\"margin-left: 0px; margin-right:0px; margin-top:0px;\">" +
                           "Determines whether or not to promote TeamSpeak usage while in game." +
                         "</blockquote>" +

                          "<h4>Maximum Number of Squad Synced players to be considered seeding</h4>" +
                           "<blockquote style=\"margin-left: 0px; margin-right:0px; margin-top:0px;\">" +
                           "If there are less than this number of players in TSSync (Team or Squad sync'ed) channel then the seeding message will be activated. (Referred to as \"seeding mode\" below)" +
                         "</blockquote>" +

                         "<h4>Seeding Recognition Message</h4>" +
                           "<blockquote style=\"margin-left: 0px; margin-right:0px; margin-top:0px;\">" +
                           "Message to display when a user joins a squad sync'ed channel when it is still considered to be in \"seeding mode\". " +
                           "<ul><li>[USER] would be replaced by the user who just joined a TSSync squad.</li><li>[TEAM] would be replaced by the team the user is on</li><li>[NEWLINE] would split the message into two messages at that point. If [NEWLINE] is not in the message and the message is larger than 127 characters then the message will automatically get split up</li></ul>" +
                         "</blockquote>" +

                         "<h4>Min Number of Team Sync players on each team to start promoting TSSync</h4>" +
                           "<blockquote style=\"margin-left: 0px; margin-right:0px; margin-top:0px;\">" +
                           "If a team has more than this number of players then the In Game promotion would be displayed to that team only." +
                         "</blockquote>" +

                         "<h4>In Game promotion interval (minutes)</h4>" +
                           "<blockquote style=\"margin-left: 0px; margin-right:0px; margin-top:0px;\">" +
                           "If the conditions are met for minimum numbers of team speaked members in a team the promotion message would be shown at this interval." +
                         "</blockquote>" +

                         "<h4>In Game Promotion Message</h4>" +
                           "<blockquote style=\"margin-left: 0px; margin-right:0px; margin-top:0px;\">" +
                           "Message shown to players for team speak in game promotion." +
                           "<ul><li>[NUMBER] would be replaced by the number of team members which are in team speak</li><li>[NEWLINE] would split the message into two messages at that point. If [NEWLINE] is not in the message and the message is larger than 127 characters then the message will automatically get split up</li></ul>" +
                         "</blockquote>";
        ...
        public List<CPluginVariable> GetDisplayPluginVariables()
        {
            ...
            lstReturn.Add(new CPluginVariable("Section 9 - TSSync Promotion|Enable TSSync Promotion", typeof(Boolean), mEnableTSPromotion));
            lstReturn.Add(new CPluginVariable("Section 9 - TSSync Promotion|Maximum Number of Squad Synced players to be considered seeding", typeof(int), mMaxSeedingRecognitionVal));
            lstReturn.Add(new CPluginVariable("Section 9 - TSSync Promotion|Seeding Recognition Message", typeof(String), mSeedingRecognitionMessage));
            lstReturn.Add(new CPluginVariable("Section 9 - TSSync Promotion|Min Number of Team Sync players on each team to start promoting TSSync", typeof(int), mMinSyncedPlayersForInGamePromotion));
        ...
        public List<CPluginVariable> GetPluginVariables()
        {
            ...
            lstReturn.Add(new CPluginVariable("Show Debug Messages (TSSync Promotion)", typeof(Boolean), dbgTSSyncPromote));
            // -- Section 8 - Event Bouncer ---------------------------------------
            ...
            lstReturn.Add(new CPluginVariable("Enable TSSync Promotion", typeof(Boolean), mEnableTSPromotion));
            lstReturn.Add(new CPluginVariable("Maximum Number of Squad Synced players to be considered seeding", typeof(int), mMaxSeedingRecognitionVal));
            lstReturn.Add(new CPluginVariable("Seeding Recognition Message", typeof(String), mSeedingRecognitionMessage));
        ...
        public void SetPluginVariable(String strVariable, String strValue)
        {
            ...
            // -- Section 7 - Debug Information -----------------------------------
                ...
                dbgBouncer = blnOut;
            else if (strVariable == "Show Debug Messages (TSSync Promotion)" && Boolean.TryParse(strValue, out blnOut))
                dbgTSSyncPromote = blnOut;
            ...
            // -- Section 9 - TSSync Promotion ----------------------------------- (PURE)
                ...
                mMaxSeedingRecognitionVal = intOut;
            else if (strVariable == "Seeding Recognition Message")
                mSeedingRecognitionMessage = strValue;
            else if (strVariable == "Min Number of Team Sync players on each team to start promoting TSSync" && Int32.TryParse(strValue, out intOut))
                mMinSyncedPlayersForInGamePromotion = intOut;
            else if (strVariable == "In Game promotion interval (minutes)" && Int32.TryParse(strValue, out intOut))
            {
                mInGamePromotionInterval = intOut;
                if (mInGamePromotionTimer.Enabled)
                {
                    debugWrite(dbgTSSyncPromote, "[TSSync Promote] - Stopping in game promotion timer.");
                    mInGamePromotionTimer.Stop();
                }
                mInGamePromotionTimer.Interval = mInGamePromotionInterval * 60 * 1000;
                if (mEnableTSPromotion)
                {
                    debugWrite(dbgTSSyncPromote, "[TSSync Promote] - Starting in game promotion timer.");
                    mInGamePromotionTimer.Start();
                }
        ...
        public void OnPluginLoaded(String strHostName, String strPort, String strPRoConVersion)
        {
            ...
            try
            {
                ...
                #region PURE In Game TSSync Promotion
                mInGamePromotionTimer = new System.Timers.Timer();
                mInGamePromotionTimer.Elapsed += new System.Timers.ElapsedEventHandler(this.inGamePromotion);
                mInGamePromotionTimer.Interval = mInGamePromotionInterval * 60 * 1000; // Convert minutes to ms.
                mInGamePromotionTimer.Stop();
                #endregion
            }
            catch (Exception e)
        ...
        /// <summary>Sends a chat message to everyone in the game. (PURE)</summary>
        public void sayToChat(String msg)
        {
            ...
            foreach (String message in messages)
            {
                ...
                {
                    int x = message.Length / MAX_MESSAGE_LENGTH;
                    for (int i = 0; i <= x; i++)
                    {
                        int length_diff = Math.Abs(MAX_MESSAGE_LENGTH - message.Substring(i * MAX_MESSAGE_LENGTH).Length);
                        String s = message.Substring(i * MAX_MESSAGE_LENGTH,
                            // For the length of the substring use the smaller of the two numbers, either the length of the remaining string or the max length of a message.
                            message.Substring(i * MAX_MESSAGE_LENGTH).Length < MAX_MESSAGE_LENGTH ? message.Substring(i * MAX_MESSAGE_LENGTH).Length : MAX_MESSAGE_LENGTH);
                        this.ExecuteCommand("procon.protected.send", "admin.say", s, "all");
                    }
                }
            }
        }
        /// <summary>Sends a chat message to a team. (PURE)</summary>
        public void sayToTeam(String msg, int teamID)
        {
            const int MAX_MESSAGE_LENGTH = 127;
            String[] stringSeparators = new string[] { "[NEWLINE]" };
            String[] messages = msg.Split(stringSeparators, StringSplitOptions.None);
                ...
                else
                {
                    ...
                    for (int i = 0; i <= x; i++)
                    {
                        ...
                        String s = message.Substring(i * MAX_MESSAGE_LENGTH,
                            // For the length of the substring use the smaller of the two numbers, either the length of the remaining string or the max length of a message.
                            message.Substring(i * MAX_MESSAGE_LENGTH).Length < MAX_MESSAGE_LENGTH ? message.Substring(i * MAX_MESSAGE_LENGTH).Length : MAX_MESSAGE_LENGTH);
                        this.ExecuteCommand("procon.protected.send", "admin.say", s, "team", "" + teamID);
                    }
                }
            }
        }
        ...
        public void updateMasterInfo()
        {
            ...
            foreach (MasterClient mstClient in mClientAllInfo)
            {
                ...
                #region PURE Update TSSynced users
                debugWrite(dbgTSSyncPromote, "[TSSync Promote] Updating TSSynced list");
                if (mstClient.HasGmClient && mstClient.HasTsClient && !mstClient.IsSyncToStaging)
                {
                    temp_mTSSyncedUsers.Add(mstClient.GmClient.Name);
                }
                #endregion
            }

            // PURE Remove anybody in mTSSyncedUsers who is no longer in game
            // It would've been easier to do mTSSyncedUsers = temp_mTSSyncedUsers but this causes issues with the promote functionality.
            foreach (String name in mTSSyncedUsers.ToArray())
            {
                if (!temp_mTSSyncedUsers.Contains(name))
                {
                    debugWrite(dbgTSSyncPromote, "[TSSync Promote] Removing " + name + " from TSSynced list.");
                    mTSSyncedUsers.Remove(name);
                }
            }
            debugWrite(dbgTSSyncPromote, "[TSSync Promote] Done updating TSSynced list. List: (" + string.Join(",", mTSSyncedUsers.ToArray()) + ")");
        ...
        public void showSeedingMessage(MasterClient client) // PURE
        {
        ...
        public void inGamePromotion(object source, System.Timers.ElapsedEventArgs e)
        {
            try
            {
                if (mEnableTSPromotion)
                {
                    ...
                    {
                        int tcount = 0;
                        tcount = getPlayersOnBothServersOnTeam(teamId).Count;
                        count += tcount;
                        debugWrite(dbgTSSyncPromote, "[TSSync Promote] - TeamId [" + teamId + "] has [" + tcount + "] players in TSSync");
                    }

                    debugWrite(dbgTSSyncPromote, "[TSSync Promote] - Total number of TSSynced players on both teams is [" + count + "]");
                    if (count >= mMinSyncedPlayersForInGamePromotion)
                    {
                        // send message to team
                        String promotionMessage = mInGamePromotionMessage;
                        promotionMessage = promotionMessage.Replace("[NUMBER]", "" + count);
                        debugWrite(dbgTSSyncPromote, "[TSSync Promote] - Displaying Promotion Message [" + promotionMessage + "]");
                        sayToChat(promotionMessage);
                    }
                }
            }
            catch (Exception e1)
            {
                consoleWrite("^8A fatal error occurred on in game promotion! Procon must be restarted for Teamspeak 3 Sync promotion to work correctly.");
                consoleWrite("^8^bMessage:^n^0 " + e1.Message);
                consoleWrite("^8^bStack Trace:^n^0 " + e1.StackTrace);
            }
        }
    }
}
